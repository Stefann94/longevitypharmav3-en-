...
export default function InvoicesPage() {
  ...
  return (
    <div>
      <h2 className={styles.heroTitle} style={{ marginBottom: '30px' }}>Invoice <strong>history</strong></h2>
      ...
      {!hasInvoices ? (
        <div className={styles.premiumCard} style={{ textAlign: 'center', padding: '60px 20px', alignItems: 'center' }}>
          ...
          <div className={styles.cardHeader}>No invoices issued yet</div>
          <p className={styles.cardContent} style={{ maxWidth: '400px', margin: '0 auto' }}>
            Invoices for your orders are generated and stored here automatically, so you can download them as PDF at any time.
          </p>
        </div>
      ) : (
        <div className={styles.premiumCard}>
          <div className={styles.cardHeader}>You have {invoices.length} invoices</div>
        </div>
      )}
    </div>
  );
