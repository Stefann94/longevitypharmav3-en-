...
export default function ReviewsPage() {
  ...
  return (
    <div>
      <h2 className={styles.heroTitle} style={{ marginBottom: '30px' }}>My <strong>reviews</strong></h2>
      ...
      {!hasReviews ? (
        <div className={styles.premiumCard} style={{ textAlign: 'center', padding: '60px 20px', alignItems: 'center' }}>
          ...
          <div className={styles.cardHeader}>You have not written a review yet</div>
          <p className={styles.cardContent} style={{ maxWidth: '400px', margin: '0 auto' }}>
            Your opinion counts. Help the LongevityPharma community by reviewing the products you have bought.
          </p>
        </div>
      ) : (
        <div className={styles.premiumCard}>
          <div className={styles.cardHeader}>You have {reviews.length} reviews</div>
        </div>
      )}
    </div>
  );
